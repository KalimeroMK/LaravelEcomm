...
use Modules\Cart\Mail\AbandonedCartSecondEmail;
use Modules\Cart\Models\AbandonedCart;

class SendAbandonedCartSecondEmailJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (! $this->abandonedCart->shouldSendSecondEmail()) {
            return;
        }
        ...
        Mail::to($email)->send(new AbandonedCartSecondEmail($this->abandonedCart));

        // Mark second email as sent
        $this->abandonedCart->update(['second_email_sent' => now()]);
    }
}
